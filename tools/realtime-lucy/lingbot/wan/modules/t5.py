...
class T5EncoderModel:
    ...
    def _encode_blockwise(self, ids, mask):
        ...
        mask = mask.to(dev)
        # umt5 (shared_pos=False) gives every block its own relative position
        # embedding — those come along inside the block's parameters below.
        # A shared one (pos_embedding is not None) is computed once here.
        e = None
        if m.pos_embedding is not None:
