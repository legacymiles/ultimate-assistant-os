...
# The website can wake a stopped pod when someone presses Render, so the pod has
# to be able to put itself back to sleep: after this many minutes with no jobs
# and no requests it stops itself (compute stops billing; the volume stays).
SERVER_ARGS+=(--idle-stop-minutes "${MUSIC_IDLE_STOP_MINUTES:-20}")
...
# autostart.sh runs at every container boot (setup.mjs gives the pod a start
# command that calls it). A stop/start wipes the container disk, so the apt
# Pythons come back first - a bootstrap re-run does that in minutes and
# downloads no weights - and then the server starts. That is what lets the
# website wake a stopped pod without anyone opening an SSH session.
AUTOSTART="$VOLUME/autostart.sh"
cat > "$AUTOSTART" <<AUTO
#!/usr/bin/env bash
# autostart.sh - generated by bootstrap.sh. Runs at pod boot; logs to $VOLUME/autostart.log
exec >> "$VOLUME/autostart.log" 2>&1
