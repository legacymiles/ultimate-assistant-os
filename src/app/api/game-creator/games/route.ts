...
import { isSkillName, isTemplateId, MAX_PROMPT_CHARS } from "@/lib/game-creator/types";
...
// POST /api/game-creator/games { prompt, template, skill? } — queue a new game.
...
export async function POST(req: Request) {
  const uid = await sessionUid();
  const body = (await req.json().catch(() => ({}))) as { prompt?: unknown; template?: unknown; skill?: unknown };
  ...
  const game = await createGame(uid, prompt, template, isSkillName(body.skill) ? body.skill : undefined);
