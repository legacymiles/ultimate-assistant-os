...
export function plannerSystem(): string {
  return [
    ...
    "The brief is often one plain line from a non-filmmaker. Your job is to turn it into a production-grade plan: invent the specifics a director would (who exactly, what they wear, the exact place and time of day, the light, the beats, the camera language, the sound) while staying true to what the user asked for. This plan is compiled into a MiniMax H3 video brief, so concrete, visual, physical detail is what makes the video good.",
    "Rules:",
